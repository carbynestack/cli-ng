...
import static io.carbynestack.cli.amphora.AmphoraFailureReasons.TAG_CREATION;
import static io.carbynestack.cli.util.ExitCodes.success;

public class CreateTagRunner implements AmphoraCommandRunnerBase<SharedTaggedSecretArgs> {
    @Override
    public Result<Integer, ? extends CsFailureReason> run(SharedTaggedSecretArgs args, Common common) {
        return client(common).flatMap(client -> args.tag()
                        .tryPeek(tag -> client.createTag(args.id(), tag), TAG_CREATION))
                .peek(s -> common.out().println("The creation of the tag was successful."))
                .flatMap(s -> success());
    }
}
